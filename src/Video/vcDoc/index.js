...
class vdDoc extends Component {
  constructor(props) {
    ...
    console.log('====================================');
    console.log('agghfgagufuyagf', this._engine);
    console.log('====================================');
    const dataBody = this.props.navigation.state.params.dataBody;
    console.log('====================================');
    console.log('khus', dataBody);
    console.log('====================================');
    this.state = {
      ...dataBody,
      joinSucceed: false,
  ...
  init = async () => {
    try {
      ...
      this._engine?.addListener(
        'JoinChannelSuccess',
        (channel, uid, elapsed) => {
          console.log('JoinChannelSuccess khush', channel, uid, elapsed);
          // Set state variable to true
        },
      );
      this._engine?.addListener('UserJoined', (uid, elapsed) => {
        console.log('UserJoined kh', uid, elapsed);
        // Get current peer IDs
        const {peerIds} = this.state;
        // If new user
        console.log('====================================');
        console.log('heelo khush', peerIds);
        console.log('====================================');
      ...
      await this._engine.adjustPlaybackSignalVolume(100);
      await this._engine.setAudioProfile(3, 1); 
      ...
      console.log('====================================');
      console.log('dsdasda', connectionState);
      console.log('====================================');
      console.log(
    ...
    try {
      ...
      if (response) {
        ...
        if (success) {
          ...
          this.props.navigation.navigate('astro_Home');
           Alert.alert('Aapke Pass',`${message}\n\nThank you for using Aapke Pass!`);
          await removeVideoCallEnd(userInfo.userId, userId);
          await removeAcceptVideoCall(userInfo.userId, userId);
          await removeVCEnd(userInfo.userId, userId);
          await removeVCEnd(userInfo.userId, userId);
  ...
  //* MuteAudio
  pauseAudio = async () => {
    console.log('doctor video pause audio');
  ...
  startAudio = async () => {
    this.init();
    console.log('doctor video start audio');
  ...
  handleMuteVideo = async () => {
    ...
    try {
      console.log('doctor video mute video');
  ...
  handleUnMuteVideo = async () => {
    try {
      this.init();
      console.log('doctor video unmute video');
                  ...
                  <CountdownCircleTimer
                    ...
                    {({remainingTime, children, animatedColor}) => (
                      <Animated.Text
                        ...
                        {(parseInt(remainingTime % 60, 10)).toString().padStart(2, '0')}
                      </Animated.Text>
                    )}
                  </CountdownCircleTimer>
                </View>
          ...
          <View style={styles.buttonHolder}>
            {this.state.muteAudio ? (
              ...
              <Touchable
                ...
                style={styles.Iconbutton}>
                <Image source={unmuteVideoCallAudio} style={styles.buttonIcon} />
              </Touchable>
            )}
