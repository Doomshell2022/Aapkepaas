...
class HomeScreen extends Component {
  ...
  componentDidMount = async fcmToken => {
    // console.log('Home Screen');
    try {
      const locationPermissionResult = await this.checkLocationPermission();
      ...
      if (locationPermissionResult === RESULTS.GRANTED) {
        ...
        if (latitude && longitude && uniqueId !== 'unknown') {
          const params = {
            latitude,
            longitude,
  ...
  showHomeData = async srtData => {
    try {
      ...
      // Handle errors gracefully without alerting the user
    }
  };
